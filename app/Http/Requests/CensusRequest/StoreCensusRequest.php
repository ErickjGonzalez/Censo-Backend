...
class StoreCensusRequest extends FormRequest
{
    ...
    {
        return [
            'name' => 'required|string|min:5|max:65|regex:/^[\pL\pM\pN\s\'\-\.\,\/\(\)]+$/u|unique:censuses,name',// Acepta mayúsculas, minúsculas, letras con acentos, números, espacios y signos básicos (',.-/())
            'description' => 'nullable|string|min:10|max:200|regex:/^[\pL\pM\pN\s\'\-\.\,\/\(\)]+$/u',// Acepta mayúsculas, minúsculas, letras con acentos, números, espacios y signos básicos (',.-/())
            'init_date' => 'required|date|after_or_equal:today',
            'deadline' => 'required|date|after:init_date',
        ];
    }
